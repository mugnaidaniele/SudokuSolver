...
def cell_to_img(digits):
    ...
    for i in range(len(digits)):
        ...
        # cv2.imwrite(os.path.join(path, "{}_{}.jpg".format(str(row), str(col))), img)
...
def draw_solution(solution, cropped, grid):
    squares = infer_grid(cropped)
    for i in range(len(squares)):
        square = squares[i]
        row, col = i // 9, i % 9
        top_left, bottom_right = square[0], square[1]

        w = int(bottom_right[0] - top_left[0])
        h = int(bottom_right[1] - top_left[1])

        if grid[row][col] == 0:
            cv2.putText(cropped, str(solution[row][col]), (int(top_left[0]) + int(w/2), int(top_left[1] + int(h/2))),
...
if __name__ == '__main__':
    ...
    original = cv2.imread("sudoku_2.jpg", cv2.IMREAD_GRAYSCALE)
    ...
    print(s.solver())
    solution = s.solver()
    solved = draw_solution(solution, cropped, grid)
    show_image(solved)
